...
exports.BRC41PaymentServiceImpl = void 0;
const crypto_1 = require("crypto");
const bsv = __importStar(require("@bsv/sdk"));
const types_1 = require("./types");
class BRC41PaymentServiceImpl {
    ...
    async initialize() {
        ...
        if (!this.serverPrivateKey) {
            ...
            this.serverPublicKey = this.serverPrivateKey.toPublicKey();
            console.warn('[BRC-41] Generated ephemeral server key - payments will not persist across restarts');
        }
        this.initialized = true;
        console.log(`[BRC-41] Payment service initialized with public key: ${this.serverPublicKey?.toString()}`);
    }
    // ==================== Payment Request Generation ====================
    async createPaymentRequest(params) {
        await this.ensureInitialized();
        if (params.satoshis < types_1.MIN_PAYMENT_SATOSHIS || params.satoshis > types_1.MAX_PAYMENT_SATOSHIS) {
            throw new types_1.BRC41Error(`Payment amount must be between ${types_1.MIN_PAYMENT_SATOSHIS} and ${types_1.MAX_PAYMENT_SATOSHIS} satoshis`, 'BRC41_INVALID_AMOUNT', 400);
        }
        const paymentId = this.generatePaymentId();
        const expires = Date.now() + types_1.PAYMENT_EXPIRY_MS;
    ...
    // ==================== Payment Processing ====================
    async processPayment(payment, paymentId) {
        await this.ensureInitialized();
        try {
            // Validate payment format
            if (payment.protocol !== types_1.BRC29_PROTOCOL_ID) {
                throw new types_1.BRC41PaymentInvalidError('Invalid BRC-29 protocol ID', 'invalid_protocol');
            }
            // Get payment request
            const paymentRecord = await this.getPaymentRecord(paymentId);
            if (!paymentRecord) {
                throw new types_1.BRC41PaymentInvalidError('Payment request not found', 'request_not_found');
            }
            if (paymentRecord.status !== 'pending') {
                throw new types_1.BRC41PaymentInvalidError('Payment request already processed', 'already_processed');
            }
            if (paymentRecord.createdAt.getTime() + types_1.PAYMENT_EXPIRY_MS < Date.now()) {
                await this.updatePaymentStatus(paymentId, 'expired');
                throw new types_1.BRC41PaymentExpiredError(paymentId);
            }
            ...
            throw new types_1.BRC41Error('Payment processing failed', 'BRC41_PROCESSING_ERROR', 500);
        }
    }
    ...
    async verifyPayment(paymentId, transactionId) {
        ...
        const paymentRecord = await this.getPaymentRecord(paymentId);
        if (!paymentRecord) {
            throw new types_1.BRC41PaymentInvalidError('Payment not found', 'payment_not_found');
        }
        if (paymentRecord.transactionId !== transactionId) {
            throw new types_1.BRC41PaymentInvalidError('Transaction ID mismatch', 'tx_mismatch');
        }
    ...
    // ==================== Pricing Calculation ====================
    calculateFee(service, usage, identityLevel) {
        ...
        return Math.max(types_1.MIN_PAYMENT_SATOSHIS, Math.round(fee));
    }
    ...
    `, [startTime, endTime]);
        ...
        return {
            ...
            paymentsByService,
            revenueByService,
            paymentsByHour: hourlyStats.map((stat) => ({
    ...
    `, [service]);
        if (!result)
            return null;
        return JSON.parse(result.pricing_config);
    }
    ...
    getServicePricingSync(service) {
        // In a real implementation, this would cache pricing configs
        // For now, return default pricing
        return types_1.DEFAULT_SERVICE_PRICING;
    }
    async verifyBRC29Payment(payment, paymentRecord) {
    ...
    async verifySPVProof(paymentRecord) {
...
exports.BRC41PaymentServiceImpl = BRC41PaymentServiceImpl;
