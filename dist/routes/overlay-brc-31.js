...
exports.overlayBrc31Router = void 0;
exports.enhancedBRC31OverlayRouter = enhancedBRC31OverlayRouter;
...
function enhancedBRC31OverlayRouter() {
    ...
    function intelligentRateLimit(maxRequests, windowMs, vipMultiplier = 2) {
        return (req, res, next) => {
    ...
    router.get('/status', (0, middleware_1.optionalBRC31Identity)(), (req, res) => {
        if (!overlayServices) {
            return res.json({
                enabled: false,
                connected: false,
    ...
    router.post('/submit', requireOverlay, (0, middleware_1.requireBRC31Identity)('verified'), // Require verified identity for submissions
    intelligentRateLimit(10, 60000), async (req, res) => {
        try {
            ...
            res.status(500).json({
                ...
                brc31: {
                    authenticated: true,
                    identityLevel: (0, middleware_1.getBRC31Identity)(req)?.level,
                },
            });
        }
    });
    // ==================== BRC-24: Lookup Services with Optional BRC-31 ====================
    router.post('/lookup', requireOverlay, (0, middleware_1.optionalBRC31Identity)(), // Optional authentication for lookups
    ...
    router.post('/files/store', requireOverlay, (0, middleware_1.requireBRC31Identity)('public-key'), // Require at least public key identity
    upload.single('file'), intelligentRateLimit(5, 60000, 2), async (req, res) => {
        try {
            if (!req.file) {
                return res.status(400).json({
                    error: 'no-file',
            ...
            const maxExpiryHours = identity.level === 'certified'
            ...
            const content = await overlayServices.brc26Service.storeFile(req.file.buffer, req.file.originalname, req.file.mimetype, {
    ...
    router.get('/files/download/:hash', (0, middleware_1.optionalBRC31Identity)(), intelligentRateLimit(50, 60000, 5), async (req, res) => {
        try {
    ...
    router.get('/brc31/status', (0, middleware_1.optionalBRC31Identity)(), async (req, res) => {
        try {
            const identity = (0, middleware_1.getBRC31Identity)(req);
            res.json({
                brc31: {
    ...
    router.post('/brc31/handshake', async (req, res) => {
        try {
            ...
            res.json({
                ...
                nonce: Buffer.from(Array.from({ length: 32 }, () => Math.floor(Math.random() * 256))).toString('base64'),
...
exports.overlayBrc31Router = enhancedBRC31OverlayRouter;
//# sourceMappingURL=overlay-brc-31.js.map