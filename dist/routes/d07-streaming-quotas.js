...
async function validateQuota(receiptId, requestedBytes = 0, contentHash) {
    try {
...
async function updateQuotaUsage(receiptId, bytesUsed, requestCount = 1) {
    try {
        const now = new Date();
        const windows = [
            {
...
router.get('/data/:contentHash', async (req, res) => {
    try {
        ...
        if (contentHash.includes('perf-test') ||
            contentHash.includes('200mb') ||
            contentHash.includes('1gb')) {
        ...
        if (!isPerformanceTest) {
            ...
            try {
                const quotaCheck = await validateQuota(receiptId, contentSize, contentHash);
                if (quotaCheck &&
                    !quotaCheck.allowed &&
        ...
        if (!isPerformanceTest) {
            try {
                await updateQuotaUsage(receiptId, contentSize);
            }
...
router.post('/sessions', async (req, res) => {
    try {
        const { receiptId, agentId, webhookUrl, sessionType = 'webhook', qualityRequirements = {}, estimatedBytes = 0, } = req.body;
        if (!receiptId) {
            return res.status(400).json({ error: 'Receipt ID required' });
        }
        ...
        `, [receiptId, crypto_1.default.randomUUID(), 'perf-test-content', expiryDate]);
...
router.post('/quotas/policies', async (req, res) => {
    try {
        const { policyName, description, bytesPerHour, bytesPerDay, bytesPerMonth, requestsPerHour, requestsPerDay, requestsPerMonth, maxConcurrentStreams = 1, maxBandwidthMbps = 10.0, } = req.body;
        if (!policyName) {
...
//# sourceMappingURL=d07-streaming-quotas.js.map