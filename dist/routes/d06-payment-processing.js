...
exports.d06PaymentProcessingRouter = d06PaymentProcessingRouter;
const crypto_1 = __importDefault(require("crypto"));
const express_1 = require("express");
const pg_1 = require("pg");
const identity_1 = require("../middleware/identity");
const brc_payment_integration_1 = require("../services/brc-payment-integration");
const bsv_payment_processor_1 = require("../services/bsv-payment-processor");
...
function d06PaymentProcessingRouter() {
    ...
    // Initialize services
    const brcIntegration = new brc_payment_integration_1.BRCPaymentIntegrationService(database, {
    ...
    const bsvProcessor = new bsv_payment_processor_1.BSVPaymentProcessor(database, {
    ...
    (0, brc_payment_integration_1.ensureBRC22Tables)(database).catch(console.error);
     ...
     */
    router.post('/pay', (0, identity_1.requireIdentity)(true), async (req, res) => {
        try {
            const { versionId, quantity = 1, paymentMethod = 'bsv', agentId, identityProof, } = req.body;
            // Validation
            ...
            const manifestResult = await database.query('SELECT version_id, content_hash, producer_id, title FROM assets WHERE version_id = $1', [versionId]);
            ...
            if (identityProof) {
                const verification = await brcIntegration.identity.verifyIdentity(identityProof.identityKey, identityProof.certificate);
    ...
    router.post('/verify', async (req, res) => {
        try {
            ...
            // Get receipt
            const receiptResult = await database.query('SELECT * FROM overlay_receipts WHERE receipt_id = $1', [receiptId]);
            if (receiptResult.rows.length === 0) {
                return json(res, 404, { error: 'receipt-not-found' });
            }
            const receipt = receiptResult.rows[0];
            ...
            // Process BSV payment
            const paymentResult = await bsvProcessor.processPayment(rawTx, receipt.total_satoshis, '76a914' + '00'.repeat(20) + '88ac');
      ...
      `, [receiptId, paymentResult.txid, 0, paymentResult.confirmations, 'confirmed']);
            ...
            if (receipt.payer_identity_key) {
                await brcIntegration.identity.updateTrustScore(receipt.payer_identity_key, true, receipt.total_satoshis);
            }
            return json(res, 200, {
                status: 'verified',
     ...
     */
    router.post('/brc31/verify', async (req, res) => {
        try {
     ...
     */
    router.post('/brc22/notify', async (req, res) => {
        try {
...
//# sourceMappingURL=d06-payment-processing.js.map