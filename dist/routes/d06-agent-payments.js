...
exports.d06AgentPaymentsRouter = d06AgentPaymentsRouter;
const express_1 = require("express");
const identity_1 = require("../middleware/identity");
const agent_payment_service_1 = require("../services/agent-payment-service");
function json(res, code, body) {
    return res.status(code).json(body);
}
function d06AgentPaymentsRouter(database) {
    const router = (0, express_1.Router)();
    const agentPaymentService = new agent_payment_service_1.AgentPaymentService(database);
    /**
     * POST /authorize - Agent payment authorization
     */
    router.post('/authorize', (0, identity_1.requireIdentity)(), async (req, res) => {
        try {
            const { agentId, maxPaymentSatoshis, dailyLimitSatoshis, monthlyLimitSatoshis, expiresAt } = req.body;
            // Validation
     ...
     */
    router.get('/:agentId/authorization', async (req, res) => {
        try {
    ...
    router.get('/:agentId/spending', async (req, res) => {
        try {
            ...
            const analytics = await agentPaymentService.getAgentSpendingAnalytics(agentId, timeframe);
            return json(res, 200, {
     ...
     */
    router.post('/:agentId/pay', async (req, res) => {
        try {
            const { agentId } = req.params;
            ...
            return json(res, 200, {
                ...
                authorization: result.authorization
                    ? {
                        ...
                        remainingLimits: {
                            dailyRemaining: result.authorization.limits.dailyLimitSatoshis -
                                result.authorization.currentUsage.dailySpentSatoshis,
                            monthlyRemaining: result.authorization.limits.monthlyLimitSatoshis -
     ...
     */
    router.post('/:agentId/suspend', (0, identity_1.requireIdentity)(), async (req, res) => {
        try {
            const { agentId } = req.params;
...
//# sourceMappingURL=d06-agent-payments.js.map