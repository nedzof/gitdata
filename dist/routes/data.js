...
const DATA_ROOT = process.env.DATA_ROOT || path_1.default.resolve(process.cwd(), 'data', 'blobs');
...
const DATA_DELIVERY_TIER = (process.env.DATA_DELIVERY_TIER || 'hot');
// Presigned URL mode: direct|presigned|stream (read dynamically for testing)
function getDataDeliveryMode() {
...
function resolveBlobPath(contentHash) {
    return path_1.default.join(DATA_ROOT, contentHash.toLowerCase());
}
/**
...
function dataRouter() {
    ...
    router.get('/v1/data', async (req, res) => {
        try {
            ...
            let range = undefined;
            if (rangeHeader) {
                range = (0, storage_1.parseRange)(rangeHeader, size);
                if (!range) {
            ...
            if (range) {
                res.status(206);
                res.setHeader('content-range', (0, storage_1.formatContentRange)(range.start, range.end, size));
