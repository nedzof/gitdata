...
exports.AdaptiveStorageCache = exports.StorageRouter = void 0;
class StorageRouter {
    constructor(pool) {
    ...
    async selectOptimalLocation(contentHash, availableLocations, clientContext, options = {}) {
        console.log(`🧭 Routing decision for ${contentHash.slice(0, 10)}... (${availableLocations.length} locations)`);
        try {
            // Score all available locations
            const scoredLocations = await Promise.all(availableLocations.map((location) => this.scoreLocation(location, clientContext, options)));
            ...
            const cacheRecommendation = await this.generateCacheRecommendation(contentHash, selectedLocation, clientContext);
            const decision = {
                selectedLocation,
    ...
    async scoreLocation(location, clientContext, options) {
        const factors = {};
        let totalScore = 0;
    ...
    calculateLatencyScore(location, clientContext, options) {
    ...
    calculateGeographicScore(location, clientContext) {
        ...
        if (preferences.length > 0) {
            const hasPreferredRegion = location.geographicRegion.some((region) => preferences.includes(region));
    ...
    calculateCostScore(location, clientContext) {
    ...
    generateRoutingReasons(scored, clientContext, options) {
        const reasons = [];
        const { location, factors } = scored;
        // Primary reason (highest scoring factor)
        const topFactor = Object.entries(factors).reduce((a, b) => factors[a[0]] > factors[b[0]] ? a : b);
        switch (topFactor[0]) {
            case 'latency':
    ...
    estimateLatency(location, clientContext) {
    ...
    estimateCost(location, clientContext) {
    ...
    async generateCacheRecommendation(contentHash, selectedLocation, clientContext) {
    ...
    async getAccessPattern(contentHash) {
    ...
    async recordRoutingDecision(contentHash, decision, clientContext) {
        ...
        await this.updatePerformanceMetrics(decision.selectedLocation, decision.estimatedLatency, clientContext);
    }
    ...
    async updatePerformanceMetrics(location, actualLatency, clientContext) {
...
class AdaptiveStorageCache {
    constructor(pool, maxMemorySizeMB = 1024) {
    ...
    async getCachedContent(contentHash, accessPattern) {
    ...
    async cacheContent(contentHash, content, metadata) {
    ...
    async updateCacheHit(contentHash, cacheLevel) {
    ...
    async updateCacheMiss(contentHash) {
    ...
    async recordCacheEntry(contentHash, cacheLevel, sizeBytes) {
        await this.pool.query(`
      INSERT INTO storage_cache_stats (
    ...
    async recordCacheEviction(contentHash, cacheLevel) {
        await this.pool.query(`
...
exports.AdaptiveStorageCache = AdaptiveStorageCache;
