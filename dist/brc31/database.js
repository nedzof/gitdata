...
exports.BRC31DatabaseService = void 0;
class QueryBuilder {
...
class BRC31DatabaseService {
    ...
    async initializeSchema() {
        ...
        await this.database.execute('CREATE INDEX IF NOT EXISTS idx_brc31_identities_level ON brc31_identities(identity_level)');
        await this.database.execute('CREATE INDEX IF NOT EXISTS idx_brc31_identities_reputation ON brc31_identities(reputation_score DESC)');
        await this.database.execute('CREATE INDEX IF NOT EXISTS idx_brc31_nonces_expires ON brc31_nonces(expires_at)');
        await this.database.execute('CREATE INDEX IF NOT EXISTS idx_brc31_nonces_identity ON brc31_nonces(identity_key, expires_at)');
    }
    // ==================== Identity Management ====================
    async storeIdentity(identity) {
        const identityData = {
            ...
            trust_metrics: JSON.stringify(identity.trustMetrics || {
                successful_auths: 0,
                failed_auths: 0,
    ...
    async updateIdentityReputation(publicKey, success) {
        // Get current metrics
        ...
        const reputationScore = Math.max(0.1, successRate * (metrics.certificate_validity_score || 1.0) * (metrics.behavioral_score || 1.0));
        const updateData = {
    ...
    async getIdentitiesByLevel(level, limit = 100) {
        const { query, params } = QueryBuilder.selectWithOptions('brc31_identities', {
    ...
    // ==================== Nonce Management ====================
    async storeNonce(nonce, identityKey, expiresAt, purpose) {
        const nonceData = {
            nonce,
    ...
    async validateAndConsumeNonce(nonce, identityKey) {
        // Check if nonce exists and is valid
        const { query: selectQuery, params: selectParams } = QueryBuilder.selectWithCustomWhere('brc31_nonces', ['*'], 'nonce = $1 AND identity_key = $2 AND used = FALSE AND expires_at > NOW()', [nonce, identityKey]);
        const nonceRecord = await this.database.queryOne(selectQuery, selectParams);
        if (!nonceRecord)
            return false;
        // Mark nonce as used
        const updateData = { used: true };
        const { query: updateQuery, params: updateParams } = QueryBuilder.update('brc31_nonces', updateData, {
            nonce,
            identity_key: identityKey,
        });
    ...
    async cleanupExpiredNonces() {
        const { query, params } = QueryBuilder.deleteWithCondition('brc31_nonces', 'expires_at < NOW() OR used = TRUE');
        const result = await this.database.execute(query, params);
        return result || 0;
    }
    async getNoncesByIdentity(identityKey, limit = 50) {
    ...
    async getAuthenticationStats() {
        const [totalIdentitiesResult, anonymousCount, publicKeyCount, verifiedCount, certifiedCount, averageReputationResult, activeNoncesResult,] = await Promise.all([
            // Total identities
            this.database.queryOne(...QueryBuilder.count('brc31_identities').query.split('').slice(0, 2)),
            // Identities by level
            this.database.queryOne(...QueryBuilder.count('brc31_identities', { identity_level: 'anonymous' })
                .query.split('')
                .slice(0, 2)),
            this.database.queryOne(...QueryBuilder.count('brc31_identities', { identity_level: 'public-key' })
                .query.split('')
                .slice(0, 2)),
            this.database.queryOne(...QueryBuilder.count('brc31_identities', { identity_level: 'verified' })
                .query.split('')
                .slice(0, 2)),
            this.database.queryOne(...QueryBuilder.count('brc31_identities', { identity_level: 'certified' })
                .query.split('')
                .slice(0, 2)),
            // Average reputation
            this.database.queryOne('SELECT AVG(reputation_score) as avg_reputation FROM brc31_identities'),
            // Active nonces
            this.database.queryOne(...QueryBuilder.selectWithCustomWhere('brc31_nonces', ['COUNT(*) as count'], 'expires_at > NOW() AND used = FALSE')
                .query.split('')
                .slice(0, 2)),
        ]);
        const totalIdentities = totalIdentitiesResult?.count || 0;
        const authenticationRate = totalIdentities > 0
            ? (await this.database.queryOne('SELECT SUM(request_count) as total_requests FROM brc31_identities'))?.total_requests / totalIdentities
            : 0;
        return {
            totalIdentities,
            identitiesByLevel: {
    ...
    async recordAuthenticationAttempt(result) {
...
exports.BRC31DatabaseService = BRC31DatabaseService;
