...
exports.runModelsMigrations = runModelsMigrations;
...
async function httpJson(method, url, body, timeoutMs = 10000) {
    ...
    try {
        ...
        if (!r.ok)
            throw new Error(`${r.status} ${JSON.stringify(js)}`);
        return js;
    }
    finally {
        clearTimeout(tm);
...
// ---------------- Migrations ----------------
async function runModelsMigrations(db) {
    if ((0, db_1.isTestEnvironment)() || db) {
        const database = db || (0, db_1.getDatabase)();
        database
            .prepare(`
...
async function upsertModel(db, r) {
...
async function searchModels(db, q, limit = 20, offset = 0) {
    const { getPostgreSQLClient } = await Promise.resolve().then(() => __importStar(require('../db/postgresql')));
    const pgClient = getPostgreSQLClient();
    const params = [];
    let whereClause = '';
...
function buildTrainingIndexManifest(p) {
    return {
        type: 'trainingIndex',
...
function buildModelArtifactManifest(modelHash, framework, sizeBytes, trainingIndexVersionId) {
    return {
        type: 'modelArtifact',
...
// ---------------- Anchor flows ----------------
async function anchorSynthetic(modelHash, tr, opts) {
...
async function anchorAdvanced(modelHash, tr, opts) {
    ...
    if (tr && (tr.parents?.length || tr.rollupHash)) {
        ...
        if (!scriptHex || !build.versionId)
            throw new Error('builder-missing (trainingIndex)');
    ...
    if (!mScriptHex || !mBuild.versionId)
        throw new Error('builder-missing (modelArtifact)');
...
function buildRawTxWithOpReturn(scriptHex) {
...
function buildLineageTree(db, versionId) {
    const visited = new Set();
    const lineageChain = [];
    function traverseLineage(currentVersionId, depth = 0) {
        if (visited.has(currentVersionId) || depth > 10)
            return; // Prevent cycles and infinite recursion
        visited.add(currentVersionId);
        // Get parents of current version
        const parents = (0, db_1.getParents)(db, currentVersionId);
        ...
        const manifest = await (0, db_1.getManifest)(currentVersionId);
        if (manifest) {
            try {
    ...
    const uniqueLineage = lineageChain.filter((item, index, arr) => arr.findIndex((other) => other.versionId === item.versionId) === index);
    return uniqueLineage;
}
// ---------------- Router ----------------
function modelsRouter(db) {
    ...
    router.get('/:id/ready', async (req, res) => {
        try {
            ...
            if (policyId) {
                try {
                    const { evaluatePolicy } = await Promise.resolve().then(() => __importStar(require('../policies')));
                    // Get policy
                    const { getPostgreSQLClient } = await Promise.resolve().then(() => __importStar(require('../db/postgresql')));
                    const pgClient = getPostgreSQLClient();
