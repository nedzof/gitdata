...
exports.D21ARCBroadcastServiceImpl = void 0;
const crypto_1 = require("crypto");
const sdk_1 = require("@bsv/sdk");
const https_1 = __importDefault(require("https"));
const types_js_1 = require("./types.js");
class D21ARCBroadcastServiceImpl {
    constructor(database, callbackBaseUrl) {
        this.providers = new Map();
        this.arcInstances = new Map();
    ...
    async initializeProviders() {
        ...
        try {
            ...
            const existingResult = await client.query('SELECT * FROM d21_arc_providers ORDER BY priority_order ASC');
            if (existingResult.rows.length === 0) {
                // No providers exist, insert defaults
                console.log('🌐 Initializing default ARC providers...');
                for (const providerConfig of types_js_1.DEFAULT_ARC_PROVIDERS) {
                    const providerId = (0, crypto_1.randomBytes)(16).toString('hex');
                    await client.query(`
          ...
          `, [
                ...
                console.log(`✅ Initialized ${types_js_1.DEFAULT_ARC_PROVIDERS.length} default ARC providers`);
            }
            else {
     ...
     */
    async initializeARCInstance(provider) {
        try {
            const arcConfig = {
                ...
                callbackToken: (0, crypto_1.randomBytes)(32).toString('hex'),
                httpClient: new sdk_1.NodejsHttpClient(https_1.default)
            };
            const arcInstance = new sdk_1.Arc(provider.apiUrl, arcConfig);
    ...
    async broadcastTransaction(request) {
        ...
        try {
            ...
            if (!provider || !provider.isActive) {
                throw new types_js_1.D21ARCError('No suitable ARC provider available', request.preferredProvider || 'unknown');
            }
            const arcInstance = this.arcInstances.get(provider.providerId);
            if (!arcInstance) {
                throw new types_js_1.D21ARCError('ARC instance not initialized for provider', provider.providerId);
            }
            console.log(`🎯 Using ARC provider: ${provider.providerName}`);
            // Create transaction from raw hex
            const transaction = sdk_1.Transaction.fromHex(request.rawTx);
            ...
            if (request.waitForStatus && formattedResponse.status !== request.waitForStatus) {
                ...
                finalResponse = await this.waitForTransactionStatus(txid, request.waitForStatus, request.maxTimeout || types_js_1.DEFAULT_ARC_CONFIG.waitForStatusTimeout);
            }
            const result = {
                txid,
            ...
            throw new types_js_1.D21ARCError(`ARC broadcast failed: ${error.message}`, request.preferredProvider || 'unknown');
        }
    }
    /**
     * Get transaction status from ARC
     */
    async getTransactionStatus(txid, providerId) {
        await this.initialize();
        const provider = providerId
            ? this.providers.get(providerId)
            : await this.selectOptimalProvider();
        if (!provider) {
            throw new types_js_1.D21ARCError('No suitable ARC provider available', providerId || 'unknown', txid);
        }
        try {
            ...
            throw new types_js_1.D21ARCError(`Failed to get transaction status: ${error.message}`, provider.providerId, txid);
        }
    }
    /**
     * Wait for transaction to reach specific status
     */
    async waitForTransactionStatus(txid, targetStatus, timeout = types_js_1.DEFAULT_ARC_CONFIG.waitForStatusTimeout) {
        const startTime = Date.now();
        while (Date.now() - startTime < timeout) {
            try {
                ...
                if (status.status === 'REJECTED' || status.status === 'DOUBLE_SPEND_ATTEMPTED') {
                    throw new types_js_1.D21ARCError(`Transaction reached terminal status: ${status.status}`, 'unknown', txid);
                }
        ...
        throw new types_js_1.D21ARCError(`Timeout waiting for transaction status: ${targetStatus}`, 'unknown', txid);
    }
    /**
     * Batch broadcast multiple transactions
     */
    async batchBroadcast(requests) {
        await this.initialize();
    ...
    async getProviderHealth(providerId) {
        ...
        if (!provider) {
            throw new types_js_1.D21ARCError('Provider not found', providerId);
        }
        try {
            const startTime = Date.now();
    ...
    async selectOptimalProvider(criteria) {
        ...
        if (availableProviders.length === 0) {
            throw new types_js_1.D21ARCError('No suitable ARC providers available', 'unknown');
        }
     ...
     */
    async getPolicyQuote(providerId) {
        const provider = this.providers.get(providerId);
        if (!provider) {
            throw new types_js_1.D21ARCError('Provider not found', providerId);
        }
        try {
            ...
            throw new types_js_1.D21ARCError(`Failed to get policy quote: ${error.message}`, providerId);
        }
    }
    /**
     * Get fee quote from ARC provider
     */
    async getFeeQuote(providerId) {
        const provider = this.providers.get(providerId);
        if (!provider) {
            throw new types_js_1.D21ARCError('Provider not found', providerId);
        }
        try {
            ...
            throw new types_js_1.D21ARCError(`Failed to get fee quote: ${error.message}`, providerId);
        }
    }
    // ==================== Private Methods ====================
    /**
    ...
    async storeARCTransaction(request, provider, response, txid) {
        const client = await this.database.getClient();
        try {
            await client.query(`
    ...
    async updateTransactionStatus(txid, statusResponse) {
        const client = await this.database.getClient();
        try {
    ...
    async updateProviderMetrics(providerId, success, responseTime) {
        const client = await this.database.getClient();
        try {
            await client.query(`
      ...
      `, [providerId, success ? 1 : 0, success ? 0 : 1, responseTime]);
            ...
            if (provider) {
                ...
                provider.averageResponseTimeMs = Math.round((provider.averageResponseTimeMs * (provider.totalBroadcasts - 1) + responseTime) /
     ...
     */
    async handleNewMerkleProof(txid, merklePath, blockHeight) {
        const client = await this.database.getClient();
        try {
...
exports.D21ARCBroadcastServiceImpl = D21ARCBroadcastServiceImpl;
exports.default = D21ARCBroadcastServiceImpl;
