...
exports.D21PaymentTemplateServiceImpl = void 0;
const crypto_1 = require("crypto");
const types_js_1 = require("./types.js");
class D21PaymentTemplateServiceImpl {
    ...
    async generateTemplate(params) {
        ...
        try {
            ...
            const outputScripts = await this.generatePaymentOutputs(params.splitRules, params.totalSatoshis, params.createdBy);
            const template = {
                templateId: (0, crypto_1.randomBytes)(16).toString('hex'),
                templateHash,
                ...
                createdBy: params.createdBy,
                createdAt: new Date(),
                expiresAt: new Date(Date.now() + types_js_1.TEMPLATE_EXPIRY_MS),
                isUsed: false,
            };
            ...
            throw new types_js_1.D21TemplateError('Failed to generate payment template', 'unknown');
        }
    }
    /**
     * Retrieve payment template by hash
     */
    async getTemplate(templateHash) {
        await this.initialize();
        const row = await this.database.queryOne(`SELECT * FROM d21_payment_templates WHERE template_hash = $1`, [templateHash]);
        if (!row) {
            return null;
        }
        return {
     ...
     */
    async verifyTemplate(templateHash) {
     ...
     */
    async getTemplateUsage(templateHash) {
        await this.initialize();
    ...
    validateSplitRules(splitRules) {
        ...
        if (Math.abs(totalSplit - 1.0) > 0.0001) {
            throw new types_js_1.D21TemplateError(`Split rules must sum to 1.0, got ${totalSplit}`, 'invalid_splits');
        }
        // Validate individual splits
        for (const [recipient, percentage] of Object.entries(splitRules)) {
            if (percentage < 0 || percentage > 1) {
                throw new types_js_1.D21TemplateError(`Split percentage for ${recipient} must be between 0 and 1, got ${percentage}`, 'invalid_split_range');
            }
        }
    }
    calculateTemplateHash(deterministicInputs) {
        ...
        const deterministicString = sortedKeys
            ...
            .join('|');
        return (0, crypto_1.createHash)('sha256').update(deterministicString).digest('hex');
    }
    async generatePaymentOutputs(splitRules, totalSatoshis, createdBy) {
        const outputs = [];
    ...
    async generateScriptForRecipient(recipient, createdBy) {
        ...
        const scriptSeed = (0, crypto_1.createHash)('sha256')
...
exports.D21PaymentTemplateServiceImpl = D21PaymentTemplateServiceImpl;
exports.default = D21PaymentTemplateServiceImpl;
