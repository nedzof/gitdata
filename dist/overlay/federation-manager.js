...
class FederationManager extends events_1.EventEmitter {
    constructor(database, nodeId, privateKey) {
        super();
    ...
    async initiateContentSync(contentHash, targetNodes, priority = 'normal') {
        try {
            const syncId = crypto_1.default.randomUUID();
            const contentSync = {
                contentHash,
    ...
    async processContentSync(syncId) {
    ...
    async registerGlobalContent(contentHash, metadata) {
        try {
            const globalContent = {
                contentHash,
    ...
    async routeContentRequest(contentHash, clientRegion) {
        try {
            ...
            const availableNodes = await Promise.all(globalContent.availableNodes.map(nodeId => this.connectedNodes.get(nodeId))).then(nodes => nodes.filter(Boolean));
    ...
    async updateSyncStatus(syncId, status, progress) {
        await this.database.execute(`
    ...
    async getNodeLoadFactor(nodeId) {
...
//# sourceMappingURL=federation-manager.js.map