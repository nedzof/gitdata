...
exports.BRC26UHRPService = void 0;
...
class BRC26UHRPService extends events_1.EventEmitter {
    constructor(database, storageBasePath, baseUrl) {
        super();
        this.myPublicKey = null;
        this.myAddress = null;
        this.advertisements = new Map();
        this.hostedContent = new Map();
    ...
    async initializeIdentity() {
        try {
            if (wallet_1.walletService.isConnected()) {
                this.myPublicKey = wallet_1.walletService.getPublicKey();
    ...
    async loadExistingContent() {
        try {
            const { query, params } = QueryBuilder.selectWithCustomWhere('uhrp_content', ['*'], 'expires_at > $1', [Date.now()]);
     ...
     */
    async storeFile(fileBuffer, filename, contentType, options = {}) {
        try {
            ...
            await fs_1.promises.mkdir(path.dirname(storagePath), { recursive: true });
            // Write file to storage
            await fs_1.promises.writeFile(storagePath, fileBuffer);
     ...
     */
    async createAdvertisement(content) {
        ...
        try {
            ...
            advertisement.signature = await wallet_1.walletService.signData(message, 'uhrp-advertisement');
     ...
     */
    async downloadContent(contentHash, hostUrl) {
        try {
            const resolution = await this.resolveContent(contentHash);
            // If we have it locally, return it
            if (resolution.content) {
                const buffer = await fs_1.promises.readFile(resolution.content.localPath);
     ...
     */
    async getFileBuffer(contentHash) {
        try {
            ...
            await this.database.execute('UPDATE uhrp_content SET download_count = download_count + 1 WHERE content_hash = $1', [contentHash]);
            return await fs_1.promises.readFile(content.localPath);
        }
        catch (error) {
    ...
    deriveAddressFromPublicKey(publicKey) {
        ...
        return (0, crypto_1.createHash)('sha256').update(publicKey).digest('base64').substring(0, 26);
    }
    createAdvertisementMessage(ad) {
        return `1UHRPYnMHPuQ5Tgb3AF8JXqwKkmZVy5hG|${ad.address}|${ad.contentHash}|${ad.url}|${ad.expiryTime}|${ad.contentLength}`;
    }
    async submitAdvertisementTransaction(ad) {
    ...
    async getHostsByPublicKeys(publicKeys) {
    ...
    async recordDownload(contentHash, hostPublicKey, downloadUrl, success, downloadTimeMs = 0, errorMessage) {
        const downloadData = {
    ...
    async cleanup() {
        try {
            ...
            const adResult = await this.database.execute('UPDATE uhrp_advertisements SET is_active = FALSE WHERE expiry_time < $1 AND is_active = TRUE', [now]);
            // Remove expired content files
            const { query: expiredQuery, params: expiredParams } = QueryBuilder.selectWithCustomWhere('uhrp_content', ['content_hash', 'local_path'], 'expires_at < $1', [now]);
            ...
            for (const content of expiredContent) {
                try {
                    await fs_1.promises.unlink(content.local_path);
...
exports.BRC26UHRPService = BRC26UHRPService;
//# sourceMappingURL=brc26-uhrp.js.map