...
exports.BRC24LookupService = void 0;
...
class BRC24LookupService extends events_1.EventEmitter {
    constructor(database, brc22Service) {
        super();
        this.lookupProviders = new Map();
     ...
     */
    removeLookupProvider(providerId) {
        this.lookupProviders.delete(providerId);
    }
    /**
     * Process a BRC-24 lookup query
     */
    async processLookup(queryRequest, requesterId) {
        try {
     ...
     */
    async checkPaymentRequirement(queryRequest, requesterId) {
     ...
     */
    async hydrateUTXOs(identifiers) {
        const utxos = [];
     ...
     * Store query record
     */
    async storeQueryRecord(queryId, queryRequest, requesterId, resultsCount) {
        const queryData = {
            query_id: queryId,
            provider: queryRequest.provider,
    ...
    async indexDatasetUTXO(topic, txid, vout, outputScript) {
    ...
    async indexPaymentUTXO(topic, txid, vout, outputScript, satoshis) {
        // Index payment information
        await this.updateProviderData('payment_tracker', topic, `${txid}:${vout}`, JSON.stringify({ outputScript, satoshis }));
    }
    async indexAgentUTXO(topic, txid, vout, outputScript) {
        // Index agent information
        await this.updateProviderData('agent_services', topic, `${txid}:${vout}`, outputScript);
    }
    async indexLineageUTXO(topic, txid, vout, outputScript) {
        // Index lineage information
        await this.updateProviderData('lineage_tracker', topic, `${txid}:${vout}`, outputScript);
    }
    async updateProviderData(providerId, topic, dataKey, dataValue) {
        const providerData = {
...
exports.BRC24LookupService = BRC24LookupService;
//# sourceMappingURL=brc24-lookup.js.map