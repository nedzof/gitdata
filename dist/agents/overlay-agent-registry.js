...
exports.OverlayAgentRegistry = void 0;
const events_1 = require("events");
class OverlayAgentRegistry extends events_1.EventEmitter {
    constructor(database, brc88Service) {
        super();
    ...
    async registerAgent(agentData) {
        ...
        if (agentData.overlayTopics.length > 0) {
            try {
                const shipAd = await this.brc88Service.createSHIPAdvertisement(agentData.overlayTopics[0]);
        ...
        let slapAdvertisementId;
        try {
            const slapAd = await this.brc88Service.createSLAPAdvertisement(`agent-service-${agentId}`);
     ...
     */
    async updateAgentPing(agentId, isUp) {
     ...
     */
    async updateAgentCapabilities(agentId, capabilities, overlayTopics) {
     ...
     */
    async recordAgentPerformance(agentId, jobId, performance) {
     ...
     */
    async updateAgentAggregateStats(agentId) {
    ...
    `, [agentId]);
      ...
      WHERE agent_id = $4
    `, [JSON.stringify(performanceStats), reputationScore, new Date(), agentId]);
    ...
    calculateReputationScore(stats) {
        ...
        if (avgTimeMs > 0) {
            if (avgTimeMs < 2000)
                timeScore = 0.2; // Fast execution bonus
            else if (avgTimeMs < 5000)
                timeScore = 0.15;
            else if (avgTimeMs < 10000)
                timeScore = 0.1;
            else if (avgTimeMs < 30000)
                timeScore = 0.05;
            else
                timeScore = 0; // Slow execution penalty
        }
     ...
     */
    async getAgentPerformanceHistory(agentId, limit = 50) {
        const results = await this.database.query(`
...
exports.OverlayAgentRegistry = OverlayAgentRegistry;
//# sourceMappingURL=overlay-agent-registry.js.map