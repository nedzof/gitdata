...
class AgentExecutionService extends events_1.EventEmitter {
     ...
     */
    async executeAgentJob(job, agent, payload) {
     ...
     */
    async callAgentWebhook(agent, payload, nonce) {
        ...
        try {
            ...
            if (this.requireIdentity && agent.identityKey) {
                ...
                if (responseHeaders) {
                    const verified = await this.verifyBRC31Signature(agent.identityKey, responseText, responseHeaders);
                    if (!verified) {
     ...
     */
    async verifyBRC31Signature(identityKey, message, headers) {
        try {
            ...
            await this.logBRC31Verification(identityKey, // Using as agent ID for logging
            identityKey, headers, isValid, isValid ? undefined : 'Signature verification failed');
            return isValid;
        }
        catch (error) {
     ...
     */
    async processExecutionResult(jobId, agentId, result) {
        ...
        if (this.requireIdentity) {
            ...
            if (agent?.identityKey) {
                const messageHash = this.createMessageHash(JSON.stringify({ jobId, agentId, result: result.artifacts }), result.nonce);
                const verified = await this.verifySignature(messageHash, result.signature, agent.identityKey);
                if (!verified) {
     ...
     */
    async processExecutionArtifacts(jobId, artifacts) {
     ...
     */
    async getAgentExecutionHistory(agentId, limit = 50) {
    ...
    `, [agentId, limit]);
        return results.map((row) => ({
            ...
            clientFeedback: typeof row.client_feedback === 'string'
    ...
    async logBRC31Verification(agentId, identityKey, headers, verified, error) {
    ...
    createMessageHash(message, nonce) {
    ...
    async signMessage(messageHash) {
        ...
        return 'mock_signature_' + (0, crypto_1.createHash)('sha256').update(messageHash).digest('hex').slice(0, 16);
    }
    async verifySignature(messageHash, signature, publicKey) {
        // In production, use proper secp256k1 signature verification
    ...
    async getAgentFromDatabase(agentId) {
        const results = await this.database.query(`
