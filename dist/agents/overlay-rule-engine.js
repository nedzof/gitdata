...
exports.OverlayRuleEngine = void 0;
const events_1 = require("events");
class OverlayRuleEngine extends events_1.EventEmitter {
    constructor(database, brc22Service, agentRegistry) {
        super();
        this.processingInterval = null;
        this.subscriptions = new Map(); // topic -> set of ruleIds
     ...
     */
    async triggerRule(ruleId, triggerEvent) {
     ...
     */
    async executeRule(rule, triggerEvent) {
        ...
        try {
            ...
            if (rule.whenCondition.predicate &&
     ...
     */
    async findAgentsForRule(rule, triggerEvent) {
    ...
    async findAgentsFromRegistry(strategy, triggerEvent) {
        ...
        if (strategy.requireAll) {
            ...
            return agents.filter((agent) => strategy.requireAll.every((requiredCap) => agent.capabilities.some((cap) => cap.name === requiredCap)));
        }
        return this.agentRegistry.searchAgents(query);
    }
    async findAgentsByCapability(strategy, triggerEvent) {
    ...
    async findAgentsFromOverlay(strategy, triggerEvent) {
     ...
     */
    async createJobsForAction(rule, action, agents, triggerEvent) {
        const jobIds = [];
        // Determine which agents can handle this action
        const eligibleAgents = agents.filter((agent) => !action.capability || agent.capabilities.some((cap) => cap.name === action.capability));
    ...
    async executeCoordinateAction(job) {
    ...
    async executeDistributeAction(job) {
     ...
     */
    async updateJobState(jobId, state, evidenceUpdate, error) {
        let sql = 'UPDATE overlay_jobs SET state = $1, updated_at = $2';
        const params = [state, new Date()];
        if (evidenceUpdate) {
            sql +=
     ...
     */
    async handleOverlayEvent(topic, eventData) {
        const subscribedRules = this.subscriptions.get(topic);
        if (!subscribedRules || subscribedRules.size === 0)
            return;
     ...
     */
    async subscribeRuleToTopics(ruleId, topics) {
    ...
    async updateRuleTriggerStats(ruleId) {
        await this.database.execute(`
    ...
    async updateRuleExecutionStats(ruleId, executionTimeMs, success) {
    ...
    async recordRuleExecution(ruleId, execution) {
        await this.database.execute(`
    ...
    mapRowToRule(row) {
        return {
            ...
            whenCondition: typeof row.when_condition === 'string'
                ...
                : row.when_condition,
            findStrategy: typeof row.find_strategy === 'string' ? JSON.parse(row.find_strategy) : row.find_strategy,
    ...
    mapRowToJob(row) {
        return {
            ...
            lineageData: typeof row.lineage_data === 'string' ? JSON.parse(row.lineage_data) : row.lineage_data,
            createdAt: parseInt(row.created_at),
            updatedAt: parseInt(row.updated_at),
        };
    }
...
exports.OverlayRuleEngine = OverlayRuleEngine;
//# sourceMappingURL=overlay-rule-engine.js.map