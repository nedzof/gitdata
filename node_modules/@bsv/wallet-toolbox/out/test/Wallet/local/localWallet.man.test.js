...
describe('localWallet tests', () => {
    ...
    test('0a monitor runOnce call history', async () => {
        ...
        (0, TestUtilsWalletStorage_1.logger)(await setup.monitor.runTask('MonitorCallHistory'));
        await setup.wallet.destroy();
    });
    ...
    test('2b create 2 nosend and sendWith', async () => {
        const setup = await (0, localWalletMethods_1.createSetup)(chain, options);
        const car = await (0, localWalletMethods_1.createOneSatTestOutput)(setup, { noSend: true }, 2);
        //await trackReqByTxid(setup, car.txid!)
        await setup.wallet.destroy();
    });
