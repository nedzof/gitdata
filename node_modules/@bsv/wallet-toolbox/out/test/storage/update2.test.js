...
describe('update2 tests', () => {
    ...
    const createDB = async (databaseName) => {
        ...
        const knexSQLite = (0, knex_1.default)({
            client: 'sqlite3',
            connection: {
    ...
    afterEach(async () => {
        for (const storage of storages) {
            if (storage instanceof index_all_1.StorageKnex) {
        ...
        for (const { storage } of setups) {
            ...
            for (const { updates } of scenarios) {
                ...
                for (const record of records) {
                    ...
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.created_at, updates.created_at, referenceTime, 10, false)).toBe(true);
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.updated_at, updates.updated_at, referenceTime, 10, false)).toBe(true);
                }
            }
        }
    });
    test('4_update ProvenTx setting individual values', async () => {
        ...
        for (const { storage } of setups) {
            ...
            try {
                ...
                expect(foundRecord === null || foundRecord === void 0 ? void 0 : foundRecord.txid).toBe('mockTxid');
            }
            catch (error) {
            ...
            expect((_a = r9.find(r => r.provenTxId === 3)) === null || _a === void 0 ? void 0 : _a.txid).toBe('mockValidTxid');
        }
    });
        ...
        for (const { storage } of setups) {
            ...
            for (const { updates } of scenarios) {
                ...
                for (const record of records) {
                    ...
                    const t = (0, index_all_1.verifyOne)(await storage.findProvenTxReqs({
                        partial: { provenTxReqId: record.provenTxReqId }
                    }));
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.created_at, updates.created_at, referenceTime)).toBe(true);
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.updated_at, updates.updated_at, referenceTime)).toBe(true);
                }
            }
        }
    });
    test('7_update ProvenTxReq setting individual values', async () => {
        ...
        for (const { storage } of setups) {
            ...
            const updatedBatch = (_a = updatedRecords.find(r => r.provenTxReqId === 3)) === null || _a === void 0 ? void 0 : _a.batch;
            ...
            expect((_b = finalRecords.find(r => r.provenTxReqId === 4)) === null || _b === void 0 ? void 0 : _b.txid).toBe('mockTxid2');
        ...
        for (const { storage } of setups) {
            ...
            for (const { description, updates } of scenarios) {
                ...
                for (const record of records) {
                    ...
                    const t = (0, index_all_1.verifyOne)(await storage.findUsers({ partial: { userId: record.userId } }));
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.created_at, updates.created_at, referenceTime)).toBe(true);
                    expect((0, TestUtilsWalletStorage_2.validateUpdateTime)(t.updated_at, updates.updated_at, referenceTime)).toBe(true);
                }
            }
    ...
    test('10_update User trigger DB unique constraint errors', async () => {
        ...
        for (const { storage } of setups) {
            ...
            const r1 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findUsers', 'updateUser', 'users', 'userId', {
                userId: 2
            });
            await expect(Promise.resolve(r1)).resolves.toBe(true);
            const r2 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findUsers', 'updateUser', 'users', 'userId', {
                identityKey: 'mockDupIdentityKey'
            });
            await expect(Promise.resolve(r2)).resolves.toBe(true);
            const r3 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findUsers', 'updateUser', 'users', 'userId', {
    ...
    test('11_update User trigger DB foreign key constraint errors', async () => {
        ...
        for (const { storage } of setups) {
            const r1 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findUsers', 'updateUser', 'users', 'userId', {
                userId: 0
            });
            await expect(Promise.resolve(r1)).resolves.toBe(true);
            const r2 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findUsers', 'updateUser', 'users', 'userId', { userId: 3 }, 0);
    ...
    test('12_update User table setting individual values', async () => {
        ...
        for (const { storage } of setups) {
            ...
            expect(updatedRecord === null || updatedRecord === void 0 ? void 0 : updatedRecord.created_at).toEqual(createdAt);
            expect(updatedRecord === null || updatedRecord === void 0 ? void 0 : updatedRecord.updated_at).toEqual(updatedAt);
        }
    });
        ...
        for (const { storage } of setups) {
            ...
            for (const { updates } of scenarios) {
                ...
                for (const record of records) {
                    ...
                    const t = (0, index_all_1.verifyOne)(await storage.findCertificates({
    ...
    test('15_update Certificate trigger DB unique constraint errors', async () => {
        ...
        for (const { storage } of setups) {
            ...
            const r1 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', { certificateId: 2 });
            await expect(Promise.resolve(r1)).resolves.toBe(true);
            const r2 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', mockDupValues);
            await expect(Promise.resolve(r2)).resolves.toBe(true);
            const r3 = await (0, TestUtilsWalletStorage_2.triggerUniqueConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', mockUniqueValues);
            await expect(Promise.resolve(r3)).resolves.toBe(false);
        }
    });
    test('16_update Certificate trigger DB foreign key constraint errors', async () => {
        ...
        for (const { storage } of setups) {
            const r1 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', { certificateId: 0 });
            await expect(Promise.resolve(r1)).resolves.toBe(true);
            const r2 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', { certificateId: 1 }, 0);
            await expect(Promise.resolve(r2)).resolves.toBe(false);
            const r3 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', { userId: 0 });
            await expect(Promise.resolve(r3)).resolves.toBe(true);
            const r4 = await (0, TestUtilsWalletStorage_2.triggerForeignKeyConstraintError)(storage, 'findCertificates', 'updateCertificate', 'certificates', 'certificateId', { userId: 1 }, 2);
            await expect(Promise.resolve(r4)).resolves.toBe(false);
        }
    });
});
//# sourceMappingURL=update2.test.js.map