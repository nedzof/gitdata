...
describe('Wallet services tests', () => {
    ...
    test('0 getUtxoStatus', async () => {
        for (const { chain, wallet, services } of ctxs) {
            if (!wallet.services || !services)
                throw new index_all_1.sdk.WERR_INTERNAL('test requires setup with services');
            {
    ...
    test('0a getUtxoStatus hashLE', async () => {
        for (const { chain, wallet, services } of ctxs) {
                ...
                throw new index_all_1.sdk.WERR_INTERNAL('test requires setup with services');
            {
                ...
                const hash = sdk_1.Utils.toHex((0, index_all_1.sha256Hash)(sdk_1.Utils.toArray(script, 'hex')));
    ...
    test('0b getUtxoStatus hashBE', async () => {
        for (const { chain, wallet, services } of ctxs) {
            ...
            {
                const script = '4104eca750b68551fb5aa893acb428b6a7d2d673498fd055cf2a8d402211b9500bdc27936846c2aa45cf82afe2f566b69cd7f7298154b0ffb25fbfa4fef8986191c4ac';
                const hash = sdk_1.Utils.toHex((0, index_all_1.sha256Hash)(sdk_1.Utils.toArray(script, 'hex')).reverse());
    ...
    test('0d getUtxoStatus outpoint', async () => {
        for (const { chain, wallet, services } of ctxs) {
                ...
                throw new index_all_1.sdk.WERR_INTERNAL('test requires setup with services');
            {
                const script = '4104eca750b68551fb5aa893acb428b6a7d2d673498fd055cf2a8d402211b9500bdc27936846c2aa45cf82afe2f566b69cd7f7298154b0ffb25fbfa4fef8986191c4ac';
                const hash = wallet.services.hashOutputScript(script);
                const us = await wallet.services.getUtxoStatus(hash, undefined, 'e4154d8ab6993addc9b8705318cc8e971dfc0780e233038ecf44c601229d93ce.0');
                if (chain === 'main') {
                    expect(us.status).toBe('success');
                    expect(us.isUtxo).toBe(true);
                }
    ...
    test('0e getUtxoStatus invalid outpoint', async () => {
        for (const { chain, wallet, services } of ctxs) {
                ...
                throw new index_all_1.sdk.WERR_INTERNAL('test requires setup with services');
            {
                const script = '4104eca750b68551fb5aa893acb428b6a7d2d673498fd055cf2a8d402211b9500bdc27936846c2aa45cf82afe2f566b69cd7f7298154b0ffb25fbfa4fef8986191c4ac';
                const hash = wallet.services.hashOutputScript(script);
                const us = await wallet.services.getUtxoStatus(hash, undefined, 'e4154d8ab6993addc9b8705318cc8e971dfc0780e233038ecf44c601229d93ce.1');
                if (chain === 'main') {
                    expect(us.status).toBe('success');
                    expect(us.isUtxo).toBe(false);
                }
