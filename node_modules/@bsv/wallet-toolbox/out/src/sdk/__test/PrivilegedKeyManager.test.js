...
const PrivilegedKeyManager_1 = require("../PrivilegedKeyManager");
const sdk_1 = require("@bsv/sdk");
const sampleData = [3, 1, 4, 1, 5, 9];
// A helper function to get a 32-byte hex
function getRandom32ByteHex() {
    const rawBytes = (0, sdk_1.Random)(32);
    return sdk_1.Utils.toHex(rawBytes);
}
describe('PrivilegedKeyManager', () => {
    it('Validates the BRC-3 compliance vector', async () => {
        const wallet = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey(1));
        const { valid } = await wallet.verifySignature({
            data: sdk_1.Utils.toArray('BRC-3 Compliance Validated!', 'utf8'),
            signature: [
    ...
    it('Validates the BRC-2 HMAC compliance vector', async () => {
        const wallet = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey('6a2991c9de20e38b31d7ea147bf55f5039e4bbc073160f5e0d541d1f17e321b8', 'hex'));
        const { valid } = await wallet.verifyHmac({
            data: sdk_1.Utils.toArray('BRC-2 HMAC Compliance Validated!', 'utf8'),
            hmac: [
    ...
    it('Validates the BRC-2 Encryption compliance vector', async () => {
        ...
        expect(sdk_1.Utils.toUTF8(plaintext)).toEqual('BRC-2 Encryption Compliance Validated!');
        await wallet.destroyKey();
    });
    it('Encrypts messages decryptable by the counterparty', async () => {
        const userKey = sdk_1.PrivateKey.fromRandom();
        const counterpartyKey = sdk_1.PrivateKey.fromRandom();
        const user = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => userKey);
        const counterparty = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => counterpartyKey);
    ...
    it('Directly signs hash of message verifiable by the counterparty', async () => {
        ...
        const counterparty = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => counterpartyKey);
        const { signature } = await user.createSignature({
            hashToDirectlySign: sdk_1.Hash.sha256(sampleData),
            protocolID: [2, 'tests'],
            keyID: '4',
            counterparty: counterpartyKey.toPublicKey().toString()
        });
        ...
        const { valid: hashValid } = await counterparty.verifySignature({
            signature,
            hashToDirectlyVerify: sdk_1.Hash.sha256(sampleData),
            protocolID: [2, 'tests'],
            keyID: '4',
            counterparty: userKey.toPublicKey().toString()
        });
    ...
    it('Uses anyone for creating signatures and self for other operations if no counterparty is provided', async () => {
        ...
        const anyone = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey(1));
    ...
    describe('PrivilegedKeyManager Key Linkage Revelation', () => {
        it('Validates the revealCounterpartyKeyLinkage function', async () => {
            // Initialize keys
            const proverKey = sdk_1.PrivateKey.fromRandom();
            const counterpartyKey = sdk_1.PrivateKey.fromRandom();
            const verifierKey = sdk_1.PrivateKey.fromRandom();
            // Initialize wallets
            const proverWallet = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => proverKey);
            const verifierWallet = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => verifierKey);
        ...
        it('Validates the revealSpecificKeyLinkage function', async () => {
            ...
            const invoiceNumberBin = sdk_1.Utils.toArray(invoiceNumber, 'utf8');
            // Compute expected linkage
            const expectedLinkage = sdk_1.Hash.sha256hmac(sharedSecret, invoiceNumberBin);
            // Compare linkage and expectedLinkage
            expect(linkage).toEqual(expectedLinkage);
            await proverWallet.destroyKey();
            await verifierWallet.destroyKey();
    ...
    describe('PrivilegedKeyManager - Internal Logic Tests', () => {
        ...
        it('Calls keyGetter only once if getPrivilegedKey is invoked multiple times within retention period', async () => {
            ...
            const keyGetterMock = jest.fn(async (reason) => {
                return new sdk_1.PrivateKey(getRandom32ByteHex(), 'hex');
            });
            // Retention period is 100 ms for testing
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(keyGetterMock, 100);
            // 1) First call, should call keyGetter once
            const key1 = await km.getPrivilegedKey('first reason');
            expect(keyGetterMock).toHaveBeenCalledTimes(1);
            // 2) Second call, if within retention, should NOT call keyGetter again
            const key2 = await km.getPrivilegedKey('second reason');
            expect(keyGetterMock).toHaveBeenCalledTimes(1);
        ...
        it('Destroys key after retention period elapses', async () => {
            ...
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(keyGetterMock, retentionMs);
            // Acquire the key
            await km.getPrivilegedKey('test reason');
            // We have chunkPropNames set
            expect(km.chunkPropNames.length).toBeGreaterThan(0);
            // Fast-forward time beyond the retention period
            jest.advanceTimersByTime(retentionMs + 1);
            // The destroyKey logic should have run
            expect(km.chunkPropNames.length).toBe(0);
            expect(km.chunkPadPropNames.length).toBe(0);
            expect(km.decoyPropNamesDestroy.length).toBe(0);
            await km.destroyKey();
        });
        it('Explicitly calls destroyKey() and removes all chunk properties', async () => {
            ...
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(keyGetterMock, 5000);
            // Acquire the key
            await km.getPrivilegedKey('destroy test');
            // Verify chunk props exist
            expect(km.chunkPropNames.length).toBeGreaterThan(0);
            expect(km.chunkPadPropNames.length).toBeGreaterThan(0);
            km.destroyKey();
        ...
        it('Reuses in-memory obfuscated key if data is valid, otherwise fetches a new key', async () => {
            ...
            const keyGetterMock = jest.fn(async () => new sdk_1.PrivateKey(mockHex, 'hex'));
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(keyGetterMock, 5000);
            // 1) First retrieval => calls keyGetter
            const key1 = await km.getPrivilegedKey('reuse test');
            expect(keyGetterMock).toHaveBeenCalledTimes(1);
            expect(key1.toHex()).toBe(mockHex);
            km[km.chunkPropNames[0]] = undefined;
            // 3) Second retrieval => chunk data is invalid => calls keyGetter again
            const key2 = await km.getPrivilegedKey('reuse test 2');
        ...
        it('Ensures chunk-splitting logic is correct for a 32-byte key', async () => {
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey(1), 5000);
            const testBytes = new Uint8Array(32);
            // Fill with some pattern, e.g. 0..31
            testBytes.forEach((_, i) => {
                testBytes[i] = i;
            });
            const chunks = km.splitKeyIntoChunks(testBytes);
            expect(chunks.length).toBe(km.CHUNK_COUNT);
            ...
            const pad = chunks.map((c) => Uint8Array.from((0, sdk_1.Random)(c.length)));
            const obfuscated = chunks.map((c, i) => km.xorBytes(c, pad[i]));
            km.chunkPropNames = [];
            km.chunkPadPropNames = [];
            obfuscated.forEach((obf, i) => {
                const chunkProp = `chunk${i}`;
                const padProp = `pad${i}`;
                km.chunkPropNames.push(chunkProp);
                km.chunkPadPropNames.push(padProp);
                km[chunkProp] = obf;
                km[padProp] = pad[i];
            });
            const reassembled = km.reassembleKeyFromChunks();
        ...
        it('XOR function works as expected', async () => {
            ...
            const result2 = km.xorBytes(a, zero);
            expect(Array.from(result2)).toEqual([0, 1, 255]);
            await km.destroyKey();
        });
        it('Generates random property names', async () => {
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey(1), 5000);
            const prop1 = km.generateRandomPropName();
            const prop2 = km.generateRandomPropName();
        ...
        it('Sets up initial decoy properties in the constructor', async () => {
            ...
            expect(km.decoyPropNamesRemain.length).toBe(2);
            // Validate those properties actually exist on the object
            for (const propName of km.decoyPropNamesRemain) {
                expect(km[propName]).toBeInstanceOf(Uint8Array);
                expect(km[propName].length).toBe(16);
            }
            await km.destroyKey();
        });
        it('New decoy properties are created on each key fetch and destroyed on destroy', async () => {
            const km = new PrivilegedKeyManager_1.PrivilegedKeyManager(async () => new sdk_1.PrivateKey(1), 5000);
            await km.getPrivilegedKey('decoy test');
            // We should have 2 decoy props that remain, plus 2 that are "destroyable"
            expect(km.decoyPropNamesRemain.length).toBe(2);
            expect(km.decoyPropNamesDestroy.length).toBe(2);
            km.destroyKey();
            expect(km.decoyPropNamesDestroy.length).toBe(0);
            await km.destroyKey();
        });
...
//# sourceMappingURL=PrivilegedKeyManager.test.js.map