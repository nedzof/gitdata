    ...
    async findNeededInputs(tx) {
        ...
        if (tx.proof === undefined) {
            for (const input of parsedTx.inputs) {
                response.requestedInputs[`${(_a = input.sourceTXID) !== null && _a !== void 0 ? _a : ''}.${input.sourceOutputIndex}`] = {
                    metadata: false
                };
            }
        ...
        const admittanceResult = await this.engine.managers[this.topic].identifyAdmissibleOutputs(parsedTx.toBEEF(), [], typeof tx.txMetadata === 'string' ? sdk_1.Utils.toArray(tx.txMetadata) : undefined);
        if (admittanceResult.outputsToAdmit.includes(tx.outputIndex)) {
            ...
            if (this.engine.managers[this.topic] !== undefined && typeof this.engine.managers[this.topic].identifyNeededInputs === 'function') {
                try {
                    const neededInputs = (_d = await ((_c = (_b = this.engine.managers[this.topic]).identifyNeededInputs) === null || _c === void 0 ? void 0 : _c.call(_b, parsedTx.toBEEF()))) !== null && _d !== void 0 ? _d : [];
    ...
    */
    ...
    getBEEFForNode(node) {
        ...
        const hydrator = (node) => {
            ...
            for (const [inputIndex, input] of tx.inputs.entries()) {
                const foundNode = this.temporaryGraphNodeRefs[`${(_a = input.sourceTXID) !== null && _a !== void 0 ? _a : ''}.${input.sourceOutputIndex}`];
                if (foundNode == null) {
