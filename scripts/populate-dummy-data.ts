...
async function populateDatabase() {
  console.log('🚀 Starting database population with realistic dummy data...');

  const dbPath = path.join(process.cwd(), 'src', 'overlay.db');

  if (!fs.existsSync(dbPath)) {
    console.error('❌ Database file not found. Please run database setup first.');
    process.exit(1);
  }
  ...
  try {
    ...
    producers.forEach(producer => {
      insertProducer.run(
        ...
        epochSeconds(Math.random() * 365) // Random time in last year
      );
    });

    console.log('📊 Inserting datasets and manifests...');
    // Insert declarations and manifests
    ...
    const insertEdge = db.prepare(`
      INSERT INTO edges (child_version_id, parent_version_id) VALUES (?, ?)
    `);
    ...
    for (const template of datasetTemplates) {
      ...
      for (let version = 0; version < numVersions; version++) {
        ...
        if (version > 0 && parentVersionIds.length > 0) {
          ...
          insertEdge.run(versionId, parentId);
        }
        ...
        insertPrice.run(versionId, price);
    ...
    const insertAdvisoryTarget = db.prepare(`
      INSERT INTO advisory_targets (advisory_id, version_id, producer_id)
      VALUES (?, ?, ?)
    `);

    datasetTemplates.forEach(template => {
      if (template.hasAdvisory) {
        ...
        const versions = db.prepare('SELECT version_id FROM manifests WHERE dataset_id = ?').all(template.dataset_id);
        versions.forEach((row: any) => {
          insertAdvisoryTarget.run(advisoryId, row.version_id, null);
        });
      }
    });
    ...
    const insertPriceRule = db.prepare(`
    ...
    insertPriceRule.run(null, 'ai-innovations', 1, 50000, now, now);
    insertPriceRule.run(null, 'ai-innovations', 10, 45000, now, now); // Volume discount
    insertPriceRule.run(null, 'financial-insights', 1, 25000, now, now);
    insertPriceRule.run(null, 'health-analytics', 1, 75000, now, now); // Premium healthcare data

    console.log('✅ Database population completed successfully!');
    console.log(`📈 Inserted:`);
    console.log(`   - ${producers.length} producers`);
    console.log(`   - ${datasetTemplates.length} dataset types`);
    console.log(`   - ${db.prepare('SELECT COUNT(*) as count FROM manifests').get().count} dataset versions`);
    console.log(`   - ${db.prepare('SELECT COUNT(*) as count FROM advisories').get().count} advisories`);
    console.log(`   - ${db.prepare('SELECT COUNT(*) as count FROM edges').get().count} lineage relationships`);
