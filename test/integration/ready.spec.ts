...
(async function run() {
  // Prepare temp headers file BEFORE loading modules
  ...
  // Clear the require cache to ensure fresh modules
  delete require.cache[require.resolve('../../src/routes/ready')];
  const { readyRouter } = require('../../src/routes/ready');
