...
describe('OP_RETURN builder and parser tests', () => {
  test('pushdata header lengths work correctly', () => {
    expect(pushdataHeaderLen(10)).toBe(1);
    expect(pushdataHeaderLen(0x4b)).toBe(1);
    expect(pushdataHeaderLen(0x4c)).toBe(2);
    expect(pushdataHeaderLen(0xff)).toBe(2);
    expect(pushdataHeaderLen(0x100)).toBe(3);
    expect(pushdataHeaderLen(0xffff)).toBe(3);
    expect(pushdataHeaderLen(0x10000)).toBe(5);
  });

  test('varInt sizing works correctly', () => {
    expect(varIntSize(10)).toBe(1);
  ...
  test('single-push and multi-push OP_RETURN scripts work', () => {
    ...
    expect(scriptHexMulti.startsWith('006a')).toBe(true);
    ...
    const o1 = outs1[0];
    expect(o1.vout).toBe(0);
    expect(o1.hasOpFalse).toBe(true);
    expect(o1.tagAscii).toBe('DLM1');
    expect(o1.pushesHex[0].toLowerCase().startsWith(Buffer.from('DLM1').toString('hex'))).toBe(true);
    ...
    expect(outs2.length).toBe(1);
    const o2 = outs2[0];
    expect(o2.hasOpFalse).toBe(true);
    expect(o2.pushesAscii[0]).toBe('DLM1');
    expect(o2.pushesHex.length >= 2).toBe(true);
  });

  test('plain OP_RETURN (no OP_FALSE) is detected', () => {
    ...
    expect(outs3.length).toBe(1);
    expect(outs3[0].hasOpFalse).toBe(false);
    expect(outs3[0].tagAscii).toBe('TRN1');
  });

  test('size estimators work correctly', () => {
    const payload = new Uint8Array([0xa0]);
    const tagged = composeTag('DLM1', payload);
    const dLen = tagged.length;
    expect(opReturnScriptLen(dLen)).toBe(2 + pushdataHeaderLen(dLen) + dLen);
    expect(opReturnOutputSize(dLen)).toBe(8 + varIntSize(opReturnScriptLen(dLen)) + opReturnScriptLen(dLen));
  });

  test('large payloads select correct push encodings', () => {
    const len4c = 0x4c; // PUSHDATA1 threshold
    const p4c = pushData(new Uint8Array(len4c));
    expect(p4c[0]).toBe(0x4c);

    const len2 = 0x100; // PUSHDATA2 threshold
    const p2 = pushData(new Uint8Array(len2));
    expect(p2[0]).toBe(0x4d);

    const len4 = 0x10000; // PUSHDATA4 threshold
    const p4 = pushData(new Uint8Array(len4));
    expect(p4[0]).toBe(0x4e);
  });
});
