...
export const certificateType: Base64String = 'jVNgF8+rifnz00856b4TkThCAvfiUE4p+t/aHYl1u0c=';
 ...
 */
export function validateCertificateFields(fields: Record<string, string>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  } else if (!fieldValidationRules.display_name.pattern.test(fields.display_name)) {
    errors.push('Display name contains invalid characters');
  }

  // Validate participant status
  if (!fields.participant || !fieldValidationRules.participant.allowedValues.includes(fields.participant)) {
    errors.push('Invalid participant status');
  }

  // Validate level if provided
  if (fields.level && !fieldValidationRules.level.allowedValues.includes(fields.level)) {
 ...
 */
export function prepareCertificateFields(inputFields: Record<string, string>): Record<string, string> {
