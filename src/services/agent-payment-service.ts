...
export interface AgentPaymentRequest {
  ...
  versionId: string;
  quantity: number;
  requestedBy?: string; // Identity key of requester
  purpose?: string; // Description of why agent needs this data
  priority?: 'low' | 'normal' | 'high';
}

export class AgentPaymentService extends EventEmitter {
  ...
  async getAgentSpendingAnalytics(
    ...
    try {
      ...
      const analytics: AgentSpendingAnalytics = {
        agentId,
   ...
   */
  async processAgentPaymentRequest(request: AgentPaymentRequest): Promise<{
    success: boolean;
    receiptId?: string;
    reason?: string;
    authorization?: AgentPaymentAuthorization;
    ...
    try {
      ...
      const { d06PaymentProcessingRouter } = await import('../routes/d06-payment-processing');
   ...
   */
  private async resetSpendingLimitsIfNeeded(agentId: string): Promise<void> {
   ...
   */
  private async getCurrentSpending(agentId: string): Promise<{
    dailySpent: number;
    monthlySpent: number;
  }> {
