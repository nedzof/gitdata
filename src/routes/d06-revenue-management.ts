...
export function d06RevenueManagementRouter(database: Pool): Router {
  const router = makeRouter();
  const revenueService = new RevenueAnalyticsService(database);
  ...
  router.get('/v1/revenue/analytics', requireIdentity(), async (req: Request, res: Response) => {
    try {
      ...
      if (!['hourly', 'daily', 'weekly', 'monthly'].includes(granularity as string)) {
        return json(res, 400, {
    ...
    try {
      ...
      switch (timeframe) {
        ...
        case 'year':
          ...
          break;
        default:
          return json(res, 400, { error: 'invalid-timeframe', hint: 'timeframe must be day, week, month, or year' });
      }
  ...
  router.get('/v1/revenue/producers/:producerId', requireIdentity(), async (req: Request, res: Response) => {
    try {
      ...
      const start = startDate ? parseDate(startDate as string) : new Date(end.getTime() - 30 * 24 * 60 * 60 * 1000);

      // Get producer information
      const producerResult = await database.query(
        'SELECT producer_id, display_name FROM producers WHERE producer_id = $1',
        [producerId]
      );
  ...
  router.post('/v1/revenue/settlements', requireIdentity(), async (req: Request, res: Response) => {
    try {
      ...
      if (receiptIds.length === 0) {
        return json(res, 400, { error: 'empty-receipt-list', hint: 'At least one receipt ID required' });
      }

      if (receiptIds.length > 1000) {
        return json(res, 400, { error: 'too-many-receipts', hint: 'Maximum 1000 receipts per settlement batch' });
      }
  ...
  router.get('/v1/revenue/settlements', requireIdentity(), async (req: Request, res: Response) => {
    try {
      ...
      `, [...parameters, parseInt(limit as string), parseInt(offset as string)]);
  ...
  router.get('/v1/revenue/export', requireIdentity(), async (req: Request, res: Response) => {
    try {
      ...
      const daysDiff = (end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24);
      if (daysDiff > 90) {
        return json(res, 400, { error: 'export-range-too-large', hint: 'Maximum 90 days for export' });
      }
