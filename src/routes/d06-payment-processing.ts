...
export function d06PaymentProcessingRouter(): Router {
  ...
  router.post('/pay', requireIdentity(true), async (req: Request, res: Response) => {
    try {
      ...
      const agentCommissionSatoshis = agentId ? Math.floor(totalSatoshis * agentCommissionPercent) : 0;
  ...
  router.post('/verify', async (req: Request, res: Response) => {
    try {
      ...
      if (!receiptId || !rawTx) {
        return json(res, 400, { error: 'missing-required-fields', hint: 'receiptId and rawTx required' });
      }
      ...
      if (receipt.status !== 'pending') {
        return json(res, 409, { error: 'receipt-already-processed', currentStatus: receipt.status });
      }
      ...
      if (!paymentResult.isValid) {
        return json(res, 400, { error: 'payment-verification-failed', reason: paymentResult.reason });
      }
