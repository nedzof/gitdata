...
export function d06AgentPaymentsRouter(database: Pool): Router {
  ...
  router.post('/authorize', requireIdentity(), async (req: Request & { identityKey?: string }, res: Response) => {
    try {
      ...
      if (!req.identityKey) {
        return json(res, 401, { error: 'identity-required', hint: 'BRC-31 identity verification required' });
      }
      ...
      if (maxPaymentSatoshis > dailyLimitSatoshis) {
        return json(res, 400, { error: 'invalid-limits', hint: 'maxPaymentSatoshis cannot exceed dailyLimitSatoshis' });
      }

      if (dailyLimitSatoshis * 31 < monthlyLimitSatoshis) {
        return json(res, 400, { error: 'invalid-limits', hint: 'monthlyLimitSatoshis seems too high relative to dailyLimitSatoshis' });
      }
  ...
  router.get('/:agentId/spending', async (req: Request, res: Response) => {
    try {
      ...
      if (!['day', 'week', 'month'].includes(timeframe)) {
        return json(res, 400, { error: 'invalid-timeframe', hint: 'timeframe must be day, week, or month' });
      }
  ...
  router.post('/:agentId/pay', async (req: Request, res: Response) => {
    try {
      ...
      if (!/^[0-9a-fA-F]{64}$/.test(versionId)) {
        return json(res, 400, { error: 'invalid-version-id', hint: 'versionId must be 64-character hex' });
      }

      if (!Number.isInteger(quantity) || quantity <= 0) {
        return json(res, 400, { error: 'invalid-quantity', hint: 'quantity must be positive integer' });
      }
      ...
      return json(res, 200, {
        ...
        authorization: result.authorization ? {
          ...
          remainingLimits: {
            dailyRemaining: result.authorization.limits.dailyLimitSatoshis - result.authorization.currentUsage.dailySpentSatoshis,
            monthlyRemaining: result.authorization.limits.monthlyLimitSatoshis - result.authorization.currentUsage.monthlySpentSatoshis
          }
        } : null,
  ...
  router.post('/:agentId/suspend', requireIdentity(), async (req: Request & { identityKey?: string }, res: Response) => {
    try {
      ...
      if (authResult.rows.length === 0) {
        return json(res, 403, { error: 'not-authorized', hint: 'Only the authorizer can suspend agent payments' });
      }
