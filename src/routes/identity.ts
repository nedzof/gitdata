...
export function initializeIdentityRoutes(database: PostgreSQLClient): Router {
  db = database;
  return router;
}
 ...
 */
router.post('/register', requireIdentity(true), async (req: IdentityRequest, res: Response) => {
  try {
    const identityKey = req.identityKey!;
    const registration: ProducerIdentityRegistration = req.body;
    ...
    const shipAdvertisementId = `ship_${crypto.randomBytes(12).toString('hex')}`;
...
router.post('/wallet/connect', async (req: Request, res: Response) => {
  try {
    const connectRequest: WalletConnectRequest = req.body;
 ...
 */
router.get('/status/:identityKey', async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/reputation', requireIdentity(false), async (req: IdentityRequest, res: Response) => {
  try {
 ...
 */
router.post('/test-verify', requireIdentity(true), async (req: IdentityRequest, res: Response) => {
  try {
