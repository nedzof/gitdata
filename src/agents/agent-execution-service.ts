...
import { OverlayAgent } from './overlay-agent-registry';
import { OverlayJob } from './overlay-rule-engine';
...
export class AgentExecutionService extends EventEmitter {
  ...
  async executeAgentJob(
    ...
    try {
      ...
      const webhookResponse = await this.callAgentWebhook(
        agent,
        webhookPayload,
        nonce
      );
  ...
  private createMessageHash(message: string, nonce: string): string {
    return createHash('sha256').update(message + nonce).digest('hex');
  }
