...
export {
  ...
  CacheKeys,
  getCacheTTLs as getRedisConnectionTTLs,
} from './redis';
...
export async function getDeclarationByTxid(txid: string): Promise<DeclarationRow | null> {
  const { getPostgreSQLClient } = await import('./postgresql');
  const pgClient = getPostgreSQLClient();
  return await pgClient.queryOne<DeclarationRow>('SELECT * FROM declarations WHERE txid = $1', [
    txid,
  ]);
}
...
export function insertAdvisoryTargets(
  db: any,
  advisoryId: string,
  targets: Array<{ version_id?: string | null; producer_id?: string | null }>,
): void;
export function insertAdvisoryTargets(
  advisoryId: string,
  targets: Array<{ version_id?: string | null; producer_id?: string | null }>,
...
export function insertAdvisoryTargets(
  dbOrAdvisoryId: any,
  advisoryIdOrTargets?: string | Array<{ version_id?: string | null; producer_id?: string | null }>,
  targets?: Array<{ version_id?: string | null; producer_id?: string | null }>,
): void | Promise<void> {
  ...
  if (dbOrAdvisoryId && dbOrAdvisoryId.prepare && typeof dbOrAdvisoryId.prepare === 'function') {
    ...
    const targetsData = advisoryIdOrTargets as Array<{
      version_id?: string | null;
      producer_id?: string | null;
    }>;
...
async function insertAdvisoryTargetsPostgreSQL(
  advisoryId: string,
  targets: Array<{ version_id?: string | null; producer_id?: string | null }>,
): Promise<void> {
  const { getPostgreSQLClient } = await import('./postgresql');
...
export async function listAdvisoriesForVersionActiveAsync(
  versionId: string,
  now: number,
): Promise<AdvisoryRow[]> {
  const { getPostgreSQLClient } = await import('./postgresql');
...
export async function listAdvisoriesForProducerActiveAsync(
  producerId: string,
  now: number,
): Promise<AdvisoryRow[]> {
  const { getPostgreSQLClient } = await import('./postgresql');
...
export function listAdvisoriesForVersionActive(
  ...
  if (!db || typeof db.prepare !== 'function') {
    console.warn(
      '[listAdvisoriesForVersionActive] This function needs to be converted to async for PostgreSQL',
    );
    return [];
  }
...
export function listAdvisoriesForProducerActive(
  ...
  if (!db || typeof db.prepare !== 'function') {
    console.warn(
      '[listAdvisoriesForProducerActive] This function needs to be converted to async for PostgreSQL',
    );
    return [];
  }
...
export function getProducerIdForVersion(db: any, versionId: string): string | null {
  if (!db || typeof db.prepare !== 'function') {
    console.warn(
      '[getProducerIdForVersion] This function needs to be converted to async for PostgreSQL',
    );
    return null;
  }
...
// Pricing functions
/* eslint-disable no-redeclare */
export async function getBestUnitPrice(
  versionId: string,
  quantity: number,
  defaultSats: number,
): Promise<{ satoshis: number; source: string; tier_from?: number }>;
export function getBestUnitPrice(
  db: any,
  versionId: string,
  quantity: number,
  defaultSats: number,
): { satoshis: number; source: string; tier_from?: number };
export function getBestUnitPrice(
  dbOrVersionId: any,
  versionIdOrQuantity?: string | number,
  ...
  if (dbOrVersionId && dbOrVersionId.prepare && typeof dbOrVersionId.prepare === 'function') {
    ...
    `);
    const ruleResult = ruleStmt.get(versionId, versionId, quantity) as
      | { satoshis: number; tier_from: number }
      | undefined;
    ...
    return getBestUnitPricePostgreSQL(
      dbOrVersionId as string,
      versionIdOrQuantity as number,
      quantityOrDefault!,
    );
  }
}

async function getBestUnitPricePostgreSQL(
  versionId: string,
  quantity: number,
  defaultSats: number,
): Promise<{ satoshis: number; source: string; tier_from?: number }> {
...
export function deletePriceRule(
  db: any,
  params: { version_id?: string; producer_id?: string; tier_from?: number | null },
): void;
export function deletePriceRule(
  versionId?: string,
  producerId?: string,
...
export function deletePriceRule(
  dbOrVersionId: any,
  paramsOrProducerId?:
    | { version_id?: string; producer_id?: string; tier_from?: number | null }
    | string,
  tierFrom?: number,
): void | Promise<void> {
...
async function deletePriceRulePostgreSQL(
  versionId?: string,
  producerId?: string,
  tierFrom?: number,
): Promise<void> {
...
export async function listJobsByRule(
  ruleId: string,
  state?: string,
  limit = 100,
  offset = 0,
...
export async function updateTemplate(
  templateId: string,
  updates: Partial<ContractTemplateRow>,
  ...
  const updatedConfig = {
    ...
    content:
      updates.template_content !== undefined ? updates.template_content : current.template_content,
    type: updates.template_type !== undefined ? updates.template_type : current.template_type,
    variables:
      updates.variables_json !== undefined
        ? JSON.parse(updates.variables_json)
        : JSON.parse(current.variables_json || 'null'),
    owner_producer_id:
      updates.owner_producer_id !== undefined
