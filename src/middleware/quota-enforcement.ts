 ...
 */
export async function validateConcurrentStreams(receiptId: string): Promise<{
  allowed: boolean;
...
export function enforceQuotas(options: {
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Validate quota
      const quotaCheck = await validateQuota(receiptId as string, estimatedBytes, windowType);
      ...
      if (checkConcurrent) {
        const concurrentCheck = await validateConcurrentStreams(receiptId as string);
