...
Establish Trust: Provide a sub-second, cryptographically secure method (/ready endpoint) for any AI pipeline to verify the lineage and integrity of a dataset before use.
Enable a Marketplace: Launch a functional producer-consumer flywheel where producers can register, price, and publish data, and consumers can discover, pay for, and download it with confidence.
Developer-First Experience: Ship a lean, powerful SDK (JS/TS) and CLI that allows a developer to integrate lineage verification and data consumption into their workflow in under 30 minutes.
...
On-Chain Anchors (Trust Layer): Bitcoin blockchain is used as an immutable ledger for DLM1, TRN1, and OTR1 commitments. This provides the ultimate, decentralized source of truth.
Overlay Service (Performance & Logic Layer): A centralized service responsible for indexing the blockchain, constructing lineage bundles, managing receipts, enforcing policies, and serving data. It is the primary interaction point for users.
Client-Side Tooling (UX Layer): The JS/TS SDK, Python Verifier, and CLIs that abstract the complexity of the protocol and provide a simple, ergonomic interface for developers.

Core User Flow (Consumer):

Consumer identifies a dataset version ID.
Calls ready(versionId) via SDK/CLI to the Overlay.
Overlay checks the lineage, policies, and advisories, returning ready: true/false.
If a purchase is required, the consumer calls pay(resource, qty).
Overlay generates a quote; consumer makes a payment, creating a TRN1 anchor.
Overlay indexes the TRN1 and issues a Receipt.
Consumer calls streamData(contentHash, receiptId).
Overlay verifies the receipt and streams the data, decrementing bandwidth.
...
8. Launch Checklist (2-Week Sprint)
 Security Hardening: Turn on and configure rate limits, body size caps, and bundle depth caps in production.
 Performance Features: Enable async proof refresh. Add confsUsed and bestHeight to /bundle and /ready responses.
 Monetization: Enforce receipt gating on /v1/data. Finalize and test bandwidth decrement accounting.
 Developer Experience: Publish final SDK and CLI packages to NPM. Create a Postman collection for the API. Write README quickstarts.
 Governance: Deploy the /advisories and /disputes endpoints. Enable advisory checks in the /ready logic.
 Operations: Set up basic abuse monitoring dashboards and alerts (e.g., spike in 4xx/5xx errors, high dispute rate).
 Documentation: Final review and publication of all public-facing documentation.
